...
// I used my own code from Lab5 for this function only.
	...
	switch(servertcb->state) {
		...
		case CONNECTED:;
			...
			while (1) {
				...
				// If length is greater than RECEIVE_BUF_SIZE, copy recvBuf to buf
				// and sleep. Repeat until length < RECEIVE_BUF_SIZE.
				while( length > RECEIVE_BUF_SIZE ) {

					if (servertcb->usedBufLen + MAX_SEG_LEN > RECEIVE_BUF_SIZE) {

						pthread_mutex_lock(servertcb->bufMutex);

						memcpy(buf + offset, servertcb->recvBuf, servertcb->usedBufLen);
						offset += servertcb->usedBufLen;
						length -= servertcb->usedBufLen;
						servertcb->usedBufLen = 0;
				...
				// If usedBufLen >= length, then copy content to buf and exit.
				if (servertcb->usedBufLen >= length) {
					...
					// Copy data to buf.
					memcpy(buf + offset, servertcb->recvBuf, length);
					servertcb->usedBufLen -= length;

					// Create a temp to hold the uncopied data in recvbuf.
					char temp[RECEIVE_BUF_SIZE];
					memcpy(temp, servertcb->recvBuf+length, servertcb->usedBufLen);

					// Clear recvbuf and copy temp to recvbuf.
					memset(servertcb->recvBuf, '\0', sizeof(char)*RECEIVE_BUF_SIZE);
					memcpy(servertcb->recvBuf, temp, servertcb->usedBufLen);
...
void* seghandler(void* arg) {
	...
	memset(&segBuf.data, 0, sizeof(MAX_SEG_LEN));
...
int savedata(svr_tcb_t* svrtcb, seg_t* segment) {
	if(segment->header.length + svrtcb->usedBufLen < RECEIVE_BUF_SIZE) {
		printf("SERVER: seqNum %d DATA RECEIVED\n", segment->header.seq_num);

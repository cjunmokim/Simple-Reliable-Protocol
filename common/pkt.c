...
int overlay_sendpkt(int nextNodeID, snp_pkt_t* pkt, int overlay_conn)
{
	...
	memset(&packet, 0, sizeof(sendpkt_arg_t));
	...
	if(send(overlay_conn,(void *)&packet, sizeof(sendpkt_arg_t), 0)<0) {
		pthread_mutex_unlock(lock);
		// // Destroy mutex.
		// if (lock != NULL) {
		// 	pthread_mutex_destroy(lock);
	...
	while(recv(network_conn,&c,1,0)>0) {
		if (state == 0) {
			...
			if(c=='#') {
				...
				// Get nextNode and packet separately.
				memcpy(nextNode, buf, sizeof(int));
				memcpy(pkt,buf+sizeof(int),sizeof(snp_pkt_t));
				return 1;
			}
			else if(c=='!') {
				buf[idx]=c;
